...
class TextSafetyClassifier:
    ...
    def load_data(self):
        self.input_ids = np.load(os.path.join(self.config['data_dir'], 'X_train_input_ids.npy'))
        self.attention_mask = np.load(os.path.join(self.config['data_dir'], 'X_train_attention_mask.npy'))
        self.y_train = np.load(os.path.join(self.config['data_dir'], 'y_train.npy'))

        print(f"训练数据加载完成。训练集大小: {len(self.input_ids)}")
        print(f"input_ids 的形状: {self.input_ids.shape}")
        print(f"attention_mask 的形状: {self.attention_mask.shape}")
    ...
    def build_model(self):
        ...
        self.config['bert_model_name'] = "D:/models/bert-base-chinese"
        model_path = self.config['bert_model_name']
        print(f"尝试加载模型的完整路径: {model_path}")
        vocab_path = os.path.join(model_path, 'vocab.txt')
        if not os.path.exists(vocab_path):
            raise ValueError(f"Tokenizer vocab.txt not found at {vocab_path}")
...
def main():
    config = {
        'data_dir': 'preprocess/20250616',
        'model_dir': os.path.join('train', 'model_transformer'),
        'basic_model_dir': r'D:\models\bert-base-chinese',
        'max_length': 128,
        'num_classes': 16,
        'learning_rate': 2e-5,
        'epochs': 10,
